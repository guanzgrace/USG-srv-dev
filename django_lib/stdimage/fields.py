...
class StdImageField(ImageField):
    ...
    def _get_thumbnail_filename(self, filename):
        ...
        splitted_filename.insert(1, '_thumbnail')
    ...
    def _rename_resize_image(self, instance=None, **kwargs):
        ...
        if getattr(instance, self.name):
            ...
            dst = self.generate_filename(instance, '%s%s' % (instance._get_pk_val(), ext))
            dst_fullpath = os.path.join(settings.MEDIA_ROOT, dst)
            if os.path.abspath(filename) != os.path.abspath(dst_fullpath) and os.path.exists(filename):
